#!/usr/bin/env bash
# ============================================================================
#  launch_glm_rl_att_unfused_moe.sh — one-command multi-node GLM-5.2 (744B) LoRA RL:
#  megatron-bridge (unfused) attention LoRA + PER-EXPERT MoE-expert LoRA
#  (shared-outer OFF, virtual-experts ON — EP-correct). TOY=1 => 5-layer single-node smoke test.
...
#     RX_GPU_COUNT=64 RX_DEVBOX_NAME=miles-exp WANDB_API_KEY=xxx bash launch_glm_rl_att_unfused_moe.sh
#     TOY=1 RX_GPU_COUNT=8 RX_DEVBOX_NAME=miles-exp WANDB_API_KEY=xxx bash launch_glm_rl_att_unfused_moe.sh   # 5-layer smoke
#
#  DSA backend default = unfused (megatron-bridge). Switch to fused: BACKEND=slime bash launch_glm_rl_att_unfused_moe.sh
#
#  Other roles (the orchestrator calls these on each pod automatically; usually no manual use):
#     bash launch_glm_rl_att_unfused_moe.sh head|worker|launch    # run on a pod
#     bash launch_glm_rl_att_unfused_moe.sh stop                  # abort the job (keep the nodes)
#
...
SCRIPT_NAME="${SCRIPT_NAME:-launch_glm_rl_att_unfused_moe.sh}"
...
# ── Model-size profile: full 744B (default) vs TOY=1 GLM-5.2 5-layer single-node smoke test ──
#   Full:  78 layers (3 dense + 75 MoE), EP32, sglang ep/dp 32, world=64.
#   TOY=1:  5 layers (3 dense + 2 MoE), single node TP8/EP8, tiny sglang, short RL run (per-expert parity smoke).
if [ "${TOY:-0}" = "1" ]; then
  NUM_LAYERS="${NUM_LAYERS:-5}"; MOE_LAYER_FREQ="${MOE_LAYER_FREQ:-[0]*3+[1]*2}"
  EP_SIZE="${EP_SIZE:-8}"
  SGL_GPUS_PER_ENGINE="${SGL_GPUS_PER_ENGINE:-2}"; SGL_EP_SIZE="${SGL_EP_SIZE:-2}"; SGL_DP_SIZE="${SGL_DP_SIZE:-2}"
  SGL_MEM_FRACTION="${SGL_MEM_FRACTION:-0.5}"; SGL_CHUNKED_PREFILL="${SGL_CHUNKED_PREFILL:-4096}"
  NUM_ROLLOUT="${NUM_ROLLOUT:-5}"; MOE_LORA_LAST_N="${MOE_LORA_LAST_N:-2}"
  NODES=1
  HF_CHECKPOINT="${HF_CHECKPOINT:-/cluster-storage/models/GLM-5.2_5layer}"
else
  NUM_LAYERS="${NUM_LAYERS:-78}"; MOE_LAYER_FREQ="${MOE_LAYER_FREQ:-[0]*3+[1]*75}"
  EP_SIZE="${EP_SIZE:-32}"
  SGL_GPUS_PER_ENGINE="${SGL_GPUS_PER_ENGINE:-32}"; SGL_EP_SIZE="${SGL_EP_SIZE:-32}"; SGL_DP_SIZE="${SGL_DP_SIZE:-32}"
  SGL_MEM_FRACTION="${SGL_MEM_FRACTION:-0.7}"; SGL_CHUNKED_PREFILL="${SGL_CHUNKED_PREFILL:-65536}"
  NUM_ROLLOUT="${NUM_ROLLOUT:-10000}"; MOE_LORA_LAST_N="${MOE_LORA_LAST_N:-10}"
...
JOB_ID="${JOB_ID:-glm5_att_unfused_moe_lora_rl_$(date +%y%m%d-%H%M%S)}"
WANDB="${WANDB:-online}"; WANDB_PROJECT="${WANDB_PROJECT:-miles-glm5_att_unfused_moe}"
...
MODEL_ARGS=(
  ...
  --moe-layer-freq "$MOE_LAYER_FREQ" --num-experts 256 --moe-router-topk 8
  ...
  --num-layers "$NUM_LAYERS" --hidden-size 6144 --ffn-hidden-size 12288 --num-attention-heads 64
...
# MoE-expert LoRA layer scope (OOM control): LoRA only the LAST $MOE_LORA_LAST_N MoE layers (default set
# in the size-profile block above: toy=2, full=10) via Megatron-Bridge ModuleMatcher wildcards
# (*.layers.<N>.*.linear_fc1/linear_fc2). Attention LoRA stays on all layers.
# NOTE: experts are grouped-GEMM TEGroupedMLP => the ONLY expert leaves are linear_fc1 (gate+up fused)
# and linear_fc2 (down); "unfused gate_proj/up_proj" does NOT exist here (they resolve to linear_fc1).
# Per-expert (this file) vs shared-outer is selected by the FLAGS below, NOT by these target names.
ATTN_TM="q_a_proj,kv_a_proj_with_mqa,q_b_proj,kv_b_proj,o_proj"
# ATTN_ONLY=1: isolation test -- attention LoRA ONLY (no MoE targets, no virtual-experts) to bisect
# whether the train-rollout drift is MoE-specific or a general LoRA-sync issue. VE_ARGS carries
# --sglang-lora-use-virtual-experts (dropped for attn-only; there is no MoE LoRA to serve).
if [ "${ATTN_ONLY:-0}" = "1" ]; then
  TARGET_MODULES="${ATTN_TM}"
  VE_ARGS=()
elif [ "${MOE_LORA_LAST_N:-0}" -gt 0 ] 2>/dev/null && [ "$MOE_LORA_LAST_N" -lt "$NUM_LAYERS" ]; then
  MOE_TM=""; for n in $(seq $((NUM_LAYERS - MOE_LORA_LAST_N)) $((NUM_LAYERS - 1))); do MOE_TM="$MOE_TM,*.layers.$n.*.linear_fc1,*.layers.$n.*.linear_fc2"; done
  TARGET_MODULES="${ATTN_TM}${MOE_TM}"
  VE_ARGS=(--sglang-lora-use-virtual-experts)
else
  TARGET_MODULES="${ATTN_TM},linear_fc1,linear_fc2"
  VE_ARGS=(--sglang-lora-use-virtual-experts)
fi
# USE_TIS=1: truncated importance sampling — off-policy correction for train<->rollout mismatch
# (reweights loss by exp(train_lp-rollout_lp) clipped to tis_clip=2.0). Mitigates the abs_diff drift.
TIS_ARGS=(); [ "${USE_TIS:-0}" = "1" ] && TIS_ARGS=(--use-tis)
...
TRAIN_ARGS=(
  ...
  # PER-EXPERT MoE LoRA: --experts-shared-outer-loras is REMOVED (shared-outer OFF => each expert gets its
  # own A/B). arguments.py forces sglang_experts_shared_outer_loras == experts_shared_outer_loras => both OFF.
  # --sglang-lora-use-virtual-experts is KEPT (SERVE side, in the sglang block below): it selects the
  # EP-correct MoE LoRA kernel; the classic (non-virtual) runner has no EP local-expert handling and
  # crashes/garbles under EP. NOTE: per-expert + virtual-experts is not yet validated at EP>1 -- run the
  # TOY=1 5-layer smoke first and watch train_rollout_logprob_abs_diff before any full run.
  ...
  --num-rollout "$NUM_ROLLOUT" --rollout-batch-size 8 --n-samples-per-prompt 16 --rollout-max-response-len 256
  ...
  # Parallelism (TP8 / EP=$EP_SIZE / PP1 / CP1); qkv-format follows the backend (unfused=bshd).
  --tensor-model-parallel-size 8 --expert-model-parallel-size "$EP_SIZE" --expert-tensor-parallel-size 1
  ...
  --rollout-num-gpus-per-engine "$SGL_GPUS_PER_ENGINE" --sglang-mem-fraction-static "$SGL_MEM_FRACTION"
  --sglang-enable-dp-attention --sglang-ep-size "$SGL_EP_SIZE" --sglang-dp-size "$SGL_DP_SIZE" --sglang-moe-dense-tp-size 1 --sglang-enable-dp-lm-head
  ...
  --sglang-chunked-prefill-size "$SGL_CHUNKED_PREFILL" --sglang-watchdog-timeout 3600
  --sglang-moe-runner-backend triton --sglang-disable-shared-experts-fusion
  --sglang-max-lora-rank 16 --sglang-lora-backend triton ${VE_ARGS[@]+"${VE_ARGS[@]}"}
...
_pod() {  # _pod <rank> "<env prefix>" <role>
  rx devbox run "$RX_DEVBOX_NAME" --rank "$1" -- bash -lc \
    "cd '$REPO_DIR' && HEAD_IP='$HEAD_IP' NODES='$NODES' GPUS_PER_NODE='$GPUS_PER_NODE' NCCL_IFNAME='$NCCL_IFNAME' RAY_PORT='$RAY_PORT' DASH_PORT='$DASH_PORT' TOY='${TOY:-0}' ATTN_ONLY='${ATTN_ONLY:-0}' USE_TIS='${USE_TIS:-0}' $2 bash '$SCRIPT_NAME' $3"
}
